...
func containers(ctx echo.Context) error {
	return ctx.JSON(http.StatusOK, HTTPContainersBody{running: "All running containers", dead: "All dead containers"})
}
...
type HTTPContainersBody struct {
	running string
	dead string
}